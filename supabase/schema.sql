...
-- The person's 3D character on the Office page (body, hair, face, clothes — the options of the office
-- kit's createPerson()), self-set from the page's avatar builder. Null until they customise it; the
-- page then picks a look from their id. Small by construction; the size check just keeps it that way.
alter table public.profiles add column if not exists office_avatar jsonb
  check (office_avatar is null or (jsonb_typeof(office_avatar) = 'object' and pg_column_size(office_avatar) <= 2048));
...
-- Saves the caller's own Office character (null resets it to the automatic look). Same reasoning
-- as update_own_profile(): only this one field, only the caller's own row, no general self-update
-- policy on profiles.
create or replace function public.set_office_avatar(new_avatar jsonb)
returns void
language plpgsql security definer set search_path = public
as $$
begin
  if new_avatar is not null and jsonb_typeof(new_avatar) <> 'object' then
    raise exception 'Avatar must be an object.';
  end if;
  update public.profiles set office_avatar = new_avatar, updated_at = now() where id = auth.uid();
end;
$$;
