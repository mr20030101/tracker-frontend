...
-- function reads every `answer` here and feeds it into the model's system prompt (see
-- supabase/functions/bot-reply/index.ts). `keywords` is no longer matched against directly (that
-- was the original keyword-only version) but is kept for now in case a future admin UI wants it for
-- categorization/search. Editable by SQL for now — no admin UI yet.
